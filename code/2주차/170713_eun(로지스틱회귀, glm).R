...
# type이 response면 확률값, class면 class로 돌려받음, 여기서의 predict.glm은 class를 지원안함
...
# 특정그룹이 나올 (확률/전체확률')이 큰 그룹으로 할당 -> 중요!!!!!!!!!!!!!!!!!!!!!!!
...
# m$softmax
# 전체 확률로 나누는 코드 -> 신경망에서 Y값이 하나가 아니라, 여러 개가 나옴
# 각 경우의 수를 확률로 계산함, 가장 높은 확률을 1로 두고 나머지는 0으로 할당함
